...
Run `python3 scripts/dag-meter.py <on|off|status>` and report its output verbatim — it already
explains what happened in plain language, including the one case worth calling out yourself if it
comes up: **turning off doesn't always take effect for the rest of the *current* session.**
