...
Phase 5 of the `orchestrator` skill, dispatched as one of a user-sized batch of concurrent
`task-worker` instances (the user picks how many at each batch's dispatch prompt, up to the run's
mode ceiling), each in its own isolated git worktree (set up by the Orchestrator's `Agent` call,
not something you need to arrange yourself).
